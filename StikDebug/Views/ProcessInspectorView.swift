...
enum ProcessControlAction: String {
    ...
    func successMessage(for pid: Int) -> String {
        switch self {
        case .resume:
            return String(format: "Sent SIGCONT (19) to PID %d.".localized, pid)
        case .pause:
            return String(format: "Sent SIGSTOP (17) to PID %d.".localized, pid)
        case .kill:
            return String(format: "PID %d was terminated.".localized, pid)
        }
    }
    ...
    func timeoutMessage(for pid: Int) -> String {
        switch self {
        case .resume:
            return String(format: "Could not confirm resume for PID %d. Try again.".localized, pid)
        case .pause:
            return String(format: "Could not confirm pause for PID %d. Try again.".localized, pid)
        case .kill:
            return String(format: "Could not confirm termination for PID %d. Try again.".localized, pid)
        }
    }
}
...
final class ProcessInspectorViewModel: ObservableObject {
    ...
    func refresh() {
        ...
        Task.detached(priority: .utility) { [weak self] in
            ...
            await MainActor.run {
                if let errorMessage {
                    self.errorAlertTitle = "Failed to Load Processes".localized
    ...
    func control(_ action: ProcessControlAction, process: ProcessInfoEntry) {
        guard activeControlState == nil else {
            actionAlertTitle = "Busy".localized
            if let activeControlState {
                actionAlertMessage = String(format: "%@ for PID %d.".localized, activeControlState.action.progressTitle, activeControlState.pid)
            } else {
                actionAlertMessage = "Another process action is already running.".localized
            }
        ...
        Task.detached(priority: .userInitiated) { [weak self] in
            ...
            let errorMessage = err?.localizedDescription ?? "Unknown error".localized
            await MainActor.run {
